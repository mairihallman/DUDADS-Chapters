...
\usepackage{amsmath, amsfonts, amssymb, bm, mathtools} % For mathematical symbols and environments
\usepackage{pgfplots}
...
% \tikzexternalize % Externalize TikZ pictures to avoid recompiling them every time
...
\usetikzlibrary{arrows.meta, matrix, positioning, fit, backgrounds}
...
\input{/Users/mairi/Desktop/code/DUDADS-Chapters/math_commands.tex}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[
      ...
      \node[below=of ret-4-3] {$\mathbf{X* Y}$};
...
\begin{figure}[h!]
    \centering
    \resizebox{0.7\textwidth}{!}{ % This resizes the TikZ figure to 70% of the text width
    \begin{tikzpicture}[
        node distance=2.5cm,
        thick, % Line thickness
        state/.style={rounded corners, minimum width=2cm, minimum height=1cm, draw, thick},
        output/.style={rounded corners, minimum width=1.5cm, minimum height=2cm, draw, thick},
        input/.style={rounded corners, minimum width=1.5cm, minimum height=2cm, draw, thick}
    ]
    
    % Folded part
    \node[state] (controller1) at (0, 0) {$h_t$};
    \node[output, above of=controller1] (circlea1) {$y_t$};
    \node[input, below of=controller1] (circleb1) {$x_t$};
    
    \draw[->] (controller1.north) -- (circlea1.south);
    \draw[->] (circleb1.north) -- (controller1.south);
    \draw[->] ($(controller1.north west)+(0.5,0)$) arc[start angle=20, end angle=330, x radius=1.2cm, y radius=1.2cm];
    
    % Unfold arrow (shortened and moved away from the controller)
    \node[draw=none] (unfoldtext) [right=1.2cm of controller1] {};
    \draw[->] ($(controller1.east)+(0.5,0)$) -- ($(controller1.east)+(2,0)$) node[midway, above] {Unfold};
    
    % Unfolded part (3 layers)
    \foreach \i/\offset in {1/-1, 2/0, 3/+1} {
        \pgfmathtruncatemacro{\tshift}{int(0 + \offset)} % Conditional sum for the subscripts
        \node[state, right=of unfoldtext, xshift=\i*2.5cm] (controller\i) {$h_{t\ifnum\i=1-1\else\ifnum\i=2\relax\else+1\fi\fi}$};
        \node[output, above of=controller\i] (circlea\i) {$y_{t\ifnum\i=1-1\else\ifnum\i=2\relax\else+1\fi\fi}$};
        \node[input, below of=controller\i] (circleb\i) {$x_{t\ifnum\i=1-1\else\ifnum\i=2\relax\else+1\fi\fi}$};
        \draw[->] (controller\i.north) -- (circlea\i.south);
        \draw[->] (circleb\i.north) -- (controller\i.south);
        
        % Connect nodes with consistent arrow lengths
        \ifnum\i>1
            \draw[->] (controller\the\numexpr\i-1.east) -- (controller\i.west);
        \fi
    }
    
    \end{tikzpicture}
    }
    \caption{Your TikZ diagram inside a figure.}
\end{figure}
